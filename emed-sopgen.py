...
#blfname = "%s_%s_%s.xlsx" % (control['blwbname'], control['blwbversion'], wbRevisionTime)
blfname = "%s_%s_%s.xlsx" % ('vsalsop', '0.9', wbFileNameTime)
wb = Workbook()

createTitleWS(wb, wbRevisionTime, str(uuid.uuid4()), generator='emed-sopgen.py', FormatWB=False)
for sheet in sheets:
	createWS(wb, sheet, dbh, FormatWB=False)
